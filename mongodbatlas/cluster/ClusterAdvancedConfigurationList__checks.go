...
func (c *jsiiProxy_ClusterAdvancedConfigurationList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_ClusterAdvancedConfigurationList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*ClusterAdvancedConfiguration:
		val := val.(*[]*ClusterAdvancedConfiguration)
	...
	case []*ClusterAdvancedConfiguration:
		val_ := val.([]*ClusterAdvancedConfiguration)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*ClusterAdvancedConfiguration; received %#v (a %T)", val, val)
		}
	}
